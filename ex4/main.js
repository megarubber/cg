...
function ellipseVertices(radiusX, radiusY, numSegments) {
  ...
  for (let i = 0; i < numSegments; i++) {
    ...
    const theta2 = ((i + 1) / numSegments) * 2 * Math.PI;
    vertices.push(0, 0);
    vertices.push(radiusX * Math.cos(theta1), radiusY * Math.sin(theta1));
    vertices.push(radiusX * Math.cos(theta2), radiusY * Math.sin(theta2));
  }
  return vertices;
}
...
function headVertices() {
  return new Float32Array(circleVertices(0.22, 24));
}

function eyeVertices() {
  return new Float32Array(ellipseVertices(0.028, 0.06, 12));
}

function antennaStickVertices() {
  return new Float32Array(rectangleVertices(0.17, 0.07, 0.024, 0.16));
}

function antennaBallVertices() {
  return new Float32Array(circleVertices(0.035, 10));
}

function neckVertices() {
  return new Float32Array(rectangleVertices(-0.055, 0.15, 0.11, 0.1));
}
...
function torsoVertices() {
  ...
  return new Float32Array(polygonVertices(points));
}
...
function speakerDotVertices() {
  ...
  for (let row = 0; row < 2; row++) {
    for (let col = 0; col < 3; col++) {
      ...
      vertices.push(...rectangleVertices(x, y, dotSize, dotSize));
    }
  }
...
function connectorVertices() {
  return new Float32Array(rectangleVertices(-0.07, -0.45, 0.14, 0.3));
}

function shoulderBlobVertices() {
  return new Float32Array(circleVertices(0.09, 16));
}
...
function handVertices() {
  return new Float32Array(circleVertices(0.055, 12));
}

function wheelVertices() {
  return new Float32Array(circleVertices(0.19, 24));
}

function hubVertices() {
  return new Float32Array(circleVertices(0.07, 16));
}
...
class ChildPart extends SceneObject {
  ...
  updateModelTransform(parentTransform) {
    const localTransform = m3.translation(this.offsetX, this.offsetY);
...
class Antenna {
  constructor() {
    this.stick = new SceneObject(
      antennaStickVertices(),
      new Float32Array([0.1, 0.1, 0.1]),
    );

    this.ball = new SceneObject(
      antennaBallVertices(),
      new Float32Array([0.1, 0.1, 0.1]),
    );
  }
  updateModelTransform(headTransform) {
    const stickLocal = m3.multiply(
      m3.translation(-0.1, 0.2),
      m3.rotation(-0.5),
    );
    this.stick.modelTransform = m3.multiply(headTransform, stickLocal);
    const ballLocal = m3.translation(0.17, 0.32);
    this.ball.modelTransform = m3.multiply(headTransform, ballLocal);
  }
  draw(renderer) {
...
class Head {
  constructor() {
    ...
    this.leftEye = new ChildPart(
      eyeVertices(),
      new Float32Array([0.05, 0.05, 0.05]),
      -0.08,
      0.02,
    );
    this.rightEye = new ChildPart(
      eyeVertices(),
      new Float32Array([0.05, 0.05, 0.05]),
      0.08,
      0.02,
  ...
  updateModelTransform(bodyTransform) {
    const localTransform = m3.translation(0.0, 0.45);
    this.headTransform = m3.multiply(bodyTransform, localTransform);
...
class Neck extends SceneObject {
  constructor() {
    super(neckVertices(), new Float32Array([0.8, 0.8, 0.8]));
  }
  updateModelTransform(bodyTransform) {
...
class Torso {
  constructor() {
    this.shield = new SceneObject(
      torsoVertices(),
      new Float32Array([0.92, 0.92, 0.92]),
    );
    this.speaker = new SceneObject(
      speakerDotVertices(),
      new Float32Array([0.1, 0.1, 0.1]),
    );
    this.connector = new SceneObject(
      connectorVertices(),
      new Float32Array([0.92, 0.92, 0.92]),
    );
  }
...
class WavingArm {
  constructor(shoulderX, shoulderY, blobColor, armColor, waveSpeed, mirror, phase) {
    ...
    this.mirror = mirror;
    this.theta = phase;
    this.blob = new SceneObject(shoulderBlobVertices(), blobColor);
    this.arm = new SceneObject(armVertices(), armColor);
    this.hand = new SceneObject(handVertices(), armColor);
  }
  updateWave() {
    this.theta += this.waveSpeed;
  }
  updateModelTransform(bodyTransform) {
    this.blob.modelTransform = m3.multiply(
      bodyTransform,
      m3.translation(this.shoulderX, this.shoulderY),
    );
    const shoulderAngle = this.mirror * (0.5 + 0.7 * Math.sin(this.theta));
    const shoulderTransform = m3.multiply(
      m3.translation(this.shoulderX, this.shoulderY),
      m3.rotation(shoulderAngle),
    );
    this.arm.modelTransform = m3.multiply(bodyTransform, shoulderTransform);
    const handLocal = m3.translation(0.0, -0.26);
    this.hand.modelTransform = m3.multiply(this.arm.modelTransform, handLocal);
  }
  draw(renderer) {
...
class Wheel {
  constructor(angularSpeed) {
    ...
    this.tire = new SceneObject(
      wheelVertices(),
      new Float32Array([0.05, 0.05, 0.05]),
    );
    this.hub = new SceneObject(
      hubVertices(),
  ...
  updateModelTransform(bodyTransform) {
    ...
    const worldTransform = m3.multiply(bodyTransform, localTransform);
